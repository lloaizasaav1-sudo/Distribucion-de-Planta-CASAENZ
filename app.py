...
# ==============================================================================
    ...
    st.markdown("Para responder con rigor al requerimiento académico, se estructuró un análisis completo de balanceo para una línea de producción dedicada al empaque de **Café de 500g**.")
    st.markdown("### 📊 Parámetros de Entrada del Sistema\n* **Jornada Laboral Única:** 8 horas/día = 480 minutos/día = **28,800 segundos/día**.\n* **Tasa de Producción Deseada ($R$):** **320 unidades/día**.")
    ...
    fig_linea = go.Figure()
    fig_linea.add_trace(go.Bar(x=df_puestos["Estación (Puesto)"], y=df_puestos["Tiempo de Ciclo Real (seg)"], name="Tiempo Utilizado", marker_color='#5C3A21'))
    fig_linea.add_trace(go.Bar(x=df_puestos["Estación (Puesto)"], y=df_puestos["Tiempo de Ocio (seg)"], name="Tiempo Muerto (Ocio)", marker_color='#D4A373'))
    fig_linea.add_hline(y=90, line_dash="dash", line_color="red", annotation_text="Límite del Ciclo (90 seg)")
    fig_linea.update_layout(title="Distribución de la Carga de Trabajo por Puesto de Operario", barmode='stack', xaxis_title="Puestos de Trabajo", yaxis_title="Segundos")
    st.plotly_chart(fig_linea, use_container_width=True)
    
    st.success("💡 **Conclusión del Balanceo:** La línea quedó balanceada con **4 estaciones** de trabajo estables. El **Puesto 2 (Tostión)** es el cuello de botella físico del sistema con 85 segundos. Contamos con una excelente eficiencia operativa del **79.17%**.")

# ==============================================================================
# PÁGINA NUEVA: CONCLUSIONES GENERALES DEL PROYECTO
# ==============================================================================
    ...
    st.markdown("""
    <div class="kpi-card">
        <h3 style='color: #4A3018; margin-top:0;'>📐 Perspectiva de la Ingeniería Industrial</h3>
        <p style='color: #333; text-align: justify; line-height: 1.6;'>
    ...
    <div class="kpi-card" style="border-left: 5px solid #D4A373;">
        <h3 style='color: #4A3018; margin-top:0;'>🚀 Viabilidad del Negocio y Soportes Futuros</h3>
        <p style='color: #333; text-align: justify; line-height: 1.6;'>
